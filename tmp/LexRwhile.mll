...
let _ = List.iter (fun (kwd, tok) -> Hashtbl.add resword_table kwd tok)
                  [("call", TOK_call);("cons", TOK_cons);("do", TOK_do);("else", TOK_else);("fi", TOK_fi);("from", TOK_from);("hd", TOK_hd);("if", TOK_if);("loop", TOK_loop);("nil", TOK_nil);("proc", TOK_proc);("return", TOK_return);("show", TOK_show);("then", TOK_then);("tl", TOK_tl);("until", TOK_until)]
