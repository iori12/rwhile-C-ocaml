...
and prtProc (i:int) (e:proc) : doc = match e with
       Proc (rident, pat1, coms, pat2) -> prPrec i 0 (concatD [render "proc" ; prtRIdent 0 rident ; render "(" ; prtPat 0 pat1 ; render ")" ; prtComListBNFC 0 coms ; render "return" ; prtPat 0 pat2])
  |    Func (rident, pat, fexp) -> prPrec i 0 (concatD [render "func" ; prtRIdent 0 rident ; render "(" ; prtPat 0 pat ; render ")" ; prtFexp 0 fexp])
...
and prtComListBNFC i es : doc = match (i, es) with
    (_,[]) -> (concatD [])
  | (_,x::xs) -> (concatD [prtCom 0 x ; render ";" ; prtComListBNFC 0 xs])
and prtThenBranch (i:int) (e:thenBranch) : doc = match e with
       BThen coms -> prPrec i 0 (concatD [render "then" ; prtComListBNFC 0 coms])
  |    BThenNone  -> prPrec i 0 (concatD [])


and prtElseBranch (i:int) (e:elseBranch) : doc = match e with
       BElse coms -> prPrec i 0 (concatD [render "else" ; prtComListBNFC 0 coms])
  |    BElseNone  -> prPrec i 0 (concatD [])


and prtDoBranch (i:int) (e:doBranch) : doc = match e with
       BDo coms -> prPrec i 0 (concatD [render "do" ; prtComListBNFC 0 coms])
  |    BDoNone  -> prPrec i 0 (concatD [])


and prtLoopBranch (i:int) (e:loopBranch) : doc = match e with
       BLoop coms -> prPrec i 0 (concatD [render "loop" ; prtComListBNFC 0 coms])
  |    BLoopNone  -> prPrec i 0 (concatD [])


and prtFexp (i:int) (e:fexp) : doc = match e with
       FIf (exp, fexp1, fexp2) -> prPrec i 0 (concatD [render "if" ; prtExp 0 exp ; render "then" ; prtFexp 0 fexp1 ; render "else" ; prtFexp 0 fexp2])
  |    Freturn exp -> prPrec i 0 (concatD [render "return" ; prtExp 0 exp])


and prtExp (i:int) (e:exp) : doc = match e with
       ENot exp -> prPrec i 0 (concatD [render "not" ; prtExp 1 exp])
  |    EAtom exp -> prPrec i 0 (concatD [render "atom" ; prtExp 1 exp])
  ...
  |    Ecall (rident, exp) -> prPrec i 0 (concatD [render "eval" ; prtRIdent 0 rident ; render "(" ; prtExp 0 exp ; render ")"])
  |    EEq (exp1, exp2) -> prPrec i 0 (concatD [render "=?" ; prtExp 1 exp1 ; prtExp 1 exp2])
  |    ELt (exp1, exp2) -> prPrec i 0 (concatD [render "<" ; prtExp 1 exp1 ; prtExp 1 exp2])
  |    ELe (exp1, exp2) -> prPrec i 0 (concatD [render "<=" ; prtExp 1 exp1 ; prtExp 1 exp2])
  |    EGt (exp1, exp2) -> prPrec i 0 (concatD [render ">" ; prtExp 1 exp1 ; prtExp 1 exp2])
  |    EGe (exp1, exp2) -> prPrec i 0 (concatD [render ">=" ; prtExp 1 exp1 ; prtExp 1 exp2])
  |    EAnd (exp1, exp2) -> prPrec i 0 (concatD [render "&&" ; prtExp 1 exp1 ; prtExp 1 exp2])
  |    EOr (exp1, exp2) -> prPrec i 0 (concatD [render "||" ; prtExp 1 exp1 ; prtExp 1 exp2])
...
and prtPat (i:int) (e:pat) : doc = match e with
       PCons (pat1, pat2) -> prPrec i 0 (concatD [render "(" ; prtPat 0 pat1 ; render "." ; prtPat 0 pat2 ; render ")"])
  |    PVar variable -> prPrec i 0 (concatD [prtVariable 0 variable])
  |    PAtom atom -> prPrec i 0 (concatD [prtAtom 0 atom])
  |    PNil  -> prPrec i 0 (concatD [render "nil"])
  |    PCall (rident, pat) -> prPrec i 0 (concatD [render "call" ; prtRIdent 0 rident ; render "(" ; prtPat 0 pat ; render ")"])
  |    PUncall (rident, pat) -> prPrec i 0 (concatD [render "uncall" ; prtRIdent 0 rident ; render "(" ; prtPat 0 pat ; render ")"])
